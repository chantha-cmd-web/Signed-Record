...
- **Rate**: ${pct}% compliance rate.`);
  ...
  return (
    <div className={`flex h-screen bg-[#F8FAFC] dark:bg-[#090D16] font-sans text-slate-900 dark:text-slate-100 overflow-hidden ${language === 'KH' ? 'font-sans' : ''}`}>
      ...
      {/* Sidebar Navigation */}
      <nav className="w-64 bg-[#0F172A] dark:bg-[#070A13] flex flex-col shrink-0 border-r border-slate-200/10 text-slate-300">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        {/* Header Bar */}
        <header className="h-16 bg-white dark:bg-[#0D1527] border-b border-slate-200 dark:border-slate-800/60 px-8 flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-950 border border-slate-200/80 dark:border-slate-800 rounded-lg p-0.5">
              <button
        ...
        <div className="space-y-8 animate-fadeIn">
          ...
          {/* Data Security & Recovery Hub */}
          <div className="bg-white dark:bg-[#0D1527] border border-slate-200 dark:border-slate-800/80 rounded-2xl p-6 space-y-6 shadow-xs animate-fadeIn">
            <div>
              <h3 className="font-black text-slate-800 dark:text-slate-100 text-sm font-display flex items-center gap-2">
                <Database className="w-4 h-4 text-indigo-500 animate-pulse" />
                {language === 'KH' ? 'ប្រព័ន្ធគ្រប់គ្រងទិន្នន័យ និងស្តារឡើងវិញ' : 'Data Security, Backup & Recovery Hub'}
              </h3>
              <p className="text-xs text-slate-500 mt-1">
                {language === 'KH' 
                  ? 'គ្រប់គ្រងទិន្នន័យលិខិតស្នាមរបស់អ្នក៖ នាំចេញ កំណត់ឡើងវិញ ឬស្តារទិន្នន័យពី Backup ដោយសុវត្ថិភាព។'
                  : 'Manage local system ledger records: download encrypted JSON backups, excel analysis spreadsheets, and restore database structures instantly.'}
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              ...
              {/* Box 1: Backup Engine */}
              <div className="bg-slate-50/50 dark:bg-slate-950/40 border border-slate-100 dark:border-slate-800/60 p-4.5 rounded-xl space-y-4 flex flex-col justify-between">
                <div>
                  <h4 className="text-xs font-black uppercase tracking-wider text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                    <Download className="w-4 h-4 text-emerald-500" />
                    {language === 'KH' ? 'នាំចេញទិន្នន័យ Backup' : '1. Secure Backup Engine'}
                  </h4>
                  <p className="text-[11px] text-slate-400 mt-1.5 font-medium leading-relaxed">
                    {language === 'KH'
                      ? 'ទាញយកទិន្នន័យទាំងអស់ជាទម្រង់ JSON សម្រាប់រក្សាទុក ឬជាឯកសារ Excel សម្រាប់វិភាគ។'
                      : 'Generate structured exports of all document transaction logs, cryptographic validation hashes, and signatures.'}
                  </p>
                </div>
                <div className="space-y-2 pt-2">
                  <button
                    onClick={handleExportJSON}
                    className="w-full py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 text-slate-700 dark:text-slate-200 text-xs font-bold rounded-lg cursor-pointer transition-colors flex items-center justify-center gap-1.5 shadow-2xs"
                  >
                    <Download className="w-3.5 h-3.5 text-slate-400" />
                    {language === 'KH' ? 'ទាញយក JSON Backup' : 'Export JSON Backup'}
                  </button>
                  <button
                  ...
                  >
                    ...
                    {language === 'KH' ? 'ទាញយកឯកសារ Excel (.xlsx)' : 'Export Excel (.xlsx)'}
                  </button>
                </div>
              </div>
              ...
              <div className="bg-slate-50/50 dark:bg-slate-950/40 border border-slate-100 dark:border-slate-800/60 p-4.5 rounded-xl space-y-4 flex flex-col justify-between">
                <div>
                  <h4 className="text-xs font-black uppercase tracking-wider text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                    ...
                    {language === 'KH' ? 'ស្តារទិន្នន័យពី Backup' : '2. Disaster Recovery'}
                  </h4>
                  <p className="text-[11px] text-slate-400 mt-1.5 font-medium leading-relaxed">
                    {language === 'KH'
                      ? 'ជ្រើសរើសឯកសារ JSON Backup ដែលធ្លាប់បាននាំចេញ ដើម្បីស្តារទិន្នន័យឡើងវិញភ្លាមៗ។'
                      : 'Restore the entire signature transaction database from a previously downloaded JSON backup file.'}
                  </p>
                </div>
                <div className="pt-2">
                  <label className="w-full py-2.5 bg-indigo-50/50 hover:bg-indigo-55 dark:bg-indigo-950/20 dark:hover:bg-indigo-950/30 border border-dashed border-indigo-250 dark:border-indigo-900/40 text-indigo-700 dark:text-indigo-400 text-xs font-black rounded-lg cursor-pointer transition-colors flex flex-col items-center justify-center gap-1">
                    <Upload className="w-4 h-4 animate-bounce" />
                    <span>{language === 'KH' ? 'ជ្រើសរើសឯកសារ JSON' : 'Upload Backup File'}</span>
                    <input
                      type="file"
                      accept=".json"
                      onChange={(e) => {
              ...
              {/* Box 3: Emergency Tools */}
              <div className="bg-slate-50/50 dark:bg-slate-950/40 border border-slate-100 dark:border-slate-800/60 p-4.5 rounded-xl space-y-4 flex flex-col justify-between">
                <div>
                  <h4 className="text-xs font-black uppercase tracking-wider text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                    <Trash2 className="w-4 h-4 text-rose-500" />
                    {language === 'KH' ? 'ឧបករណ៍សង្គ្រោះបន្ទាន់' : '3. Security Controls'}
                  </h4>
                  <p className="text-[11px] text-slate-400 mt-1.5 font-medium leading-relaxed">
                    {language === 'KH'
                      ? 'លុបទិន្នន័យលិខិតស្នាមទាំងអស់ចេញពីគណនីរបស់អ្នក ឬស្ដារទិន្នន័យគំរូដើមឡើងវិញ។'
                      : 'Wipe database storage clean to fulfill privacy purge mandates, or reload pristine seed compliance records.'}
                  </p>
                </div>
                <div className="space-y-2 pt-2">
                  <button
                    onClick={handleResetDatabase}
                    className="w-full py-2 bg-slate-200 hover:bg-slate-300 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-bold rounded-lg cursor-pointer transition-colors flex items-center justify-center gap-1.5 shadow-2xs"
                  >
                    <RefreshCw className="w-3.5 h-3.5 text-slate-400" />
                    {language === 'KH' ? 'ស្ដារទិន្នន័យគំរូដើម' : 'Restore Seed Records'}
                  </button>
                  <button
                    onClick={handleClearAllData}
                    className="w-full py-2 bg-rose-50 hover:bg-rose-100 dark:bg-rose-950/20 dark:hover:bg-rose-950/30 text-rose-700 dark:text-rose-400 border border-rose-100 dark:border-rose-900/30 text-xs font-black rounded-lg cursor-pointer transition-colors flex items-center justify-center gap-1.5"
                  >
                    <Trash2 className="w-3.5 h-3.5" />
                    {language === 'KH' ? 'សម្អាតទិន្នន័យទាំងអស់' : 'Wipe All Data Clean'}
                  </button>
                </div>
              </div>
          ...
          <div className="bg-white dark:bg-[#0D1527] border border-slate-200 dark:border-slate-800/80 rounded-2xl flex flex-col overflow-hidden shadow-xs">
            ...
            {/* Sub-tab navigation */}
            <div className="px-6 pt-4 border-b border-slate-100 dark:border-slate-800/80 flex items-center justify-between">
              <div className="flex gap-4">
                ...
                >
                  {language === 'KH' ? 'លិខិតចុះហត្ថលេខារួច' : 'Signed Document Logs'} ({records.filter(r => r.signatureStatus === 'Completed' || r.signatureStatus === 'Approved & Signed').length})
                </button>
                <button
                ...
                >
                  {language === 'KH' ? 'លិខិតកំពុងរង់ចាំ' : 'Pending Signature Logs'} ({records.filter(r => r.signatureStatus !== 'Completed' && r.signatureStatus !== 'Approved & Signed').length})
                </button>
              </div>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-slate-50/60 dark:bg-slate-950/20 border-b border-slate-100 dark:border-slate-800/60">
                    ...
                    <th className="px-6 py-3.5 text-[9px] font-extrabold text-slate-400 uppercase tracking-widest w-32 text-right">Action</th>
                  </tr>
                </thead>
                <tbody>
                  {(() => {
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-right">
                          <button
                            ...
                            className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-lg text-[10px] font-extrabold transition-all cursor-pointer inline-flex items-center gap-1 border border-slate-200/40 dark:border-slate-700/50"
                          >
                            {language === 'KH' ? 'ពិនិត្យ' : 'View'}
                          </button>
                        </td>
                      </tr>
                    ));
